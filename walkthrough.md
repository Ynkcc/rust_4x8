...
## 4x4 变体评估结果（2026-08-19，n = 100, Seed = 42）

> 说明：在 4x2 评估之后，新增 4x4 棋盘变体的基线对比测试。此处 `last.pt` 为 4x4 的**冷启动模型**（尚未自对弈强化）。

### 4x4 变体 5 组对比测试结果（Seed = 42, n = 100, A 视角）

| 序号 | 选手 A vs 选手 B | 对战结果 (A 视角) | A 视角胜率 | 解读 |
| :--- | :--- | :--- | :--- | :--- |
| **g1** | `mcts128` vs `mcts128` | 胜 48 \| 平 25 \| 负 27 | **48.0%** | 同强度对称，接近五五开 ✅ |
| **g2** | `random` vs `random` | 胜 17 \| 平 70 \| 负 13 | **17.0%** | 随机对随机大量平局，符合预期 ✅ |
| **g3** | `random` vs `mcts128` | 胜 5 \| 平 9 \| 负 86 | **5.0%** | random 被规则碾压，合理 ✅ |
| **g4** | `random` vs `last.pt`（冷启动） | 胜 2 \| 平 1 \| 负 97 | **2.0%** | 冷启动模型碾压 random ✅ |
| **g5** | `last.pt`（冷启动）vs `mcts128` | 胜 39 \| 平 17 \| 负 44 | **39.0%** | 冷启动模型 vs 强规则，接近但略逊 |

### 结论

1. **基线测试（g1-g3）全部正常**：引擎对弈、平局处理、规则强度都符合预期。
2. **冷启动模型（last.pt）强度**：
   - **vs random 胜率 98%**（g4：2/1/97）—— 模型已远超随机乱走，具备基本攻防常识 ✅
   - **vs mcts128 胜率 39%**（g5：39/17/44）—— 距强规则对手（MCTS 128 sims）尚有一定差距，但已能与之周旋（非一边倒）。这与之前诊断一致——价值头强、策略头仍需强化。。