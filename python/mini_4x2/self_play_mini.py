"""
self_play_mini.py — 4x2 迷你暗棋的自对弈数据生产者

调用 PyO3 的 `run_mini_*_self_play_with_predictor` 驱动 Gumbel MCTS 自对弈，
生成训练局数据并压入数据队列。仅服务于 4x2 迷你环境（网络维度与动作空间均取自
constant_mini.py / banqi_4x8.MINI_*）。
"""
...
class PredictorMini:
    ...
    def __init__(self, model: "MiniBanqiNet", device: "torch.device", model_path: str | None) -> None:
    ...
    def _maybe_reload_weights(self, force: bool = False) -> None:
        ...
        if force or mtime > self._mtime:
            try:
                ...
                self.model.eval()
                self._mtime = mtime
                print(f"[PredictorMini] 已重载权重: {self.model_path}")
            except Exception as exc:  # pragma: no cover
                print(f"[PredictorMini] 权重加载失败 (保持旧模型): {exc}")

    def __call__(self, board: np.ndarray, scalars: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        """返回 (policy_logits (N,40) float32, values (N,) float32)。"""
...
class SelfPlayWorkerMini(threading.Thread):
    """生产者线程：调用 run_mini_* 生成 episode，压入数据队列（无 Mongo 归档）。"""

    def __init__(
        self,
        predictor: PredictorMini,
        sp_cfg,
        data_q: "queue.Queue",
        stop_flag: "List[bool]",
        worker_id: int = 0,
    ) -> None:
        super().__init__(name="SelfPlayWorkerMini", daemon=True)
    ...
    def run(self) -> None:
        while not self.stop_flag[0]:
            ...
            if config.NUM_WORKERS > 1:
                episodes = banqi_4x8.run_mini_parallel_self_play_with_predictor(
                ...
                episodes = banqi_4x8.run_mini_self_play_with_predictor(
                    predict_fn=self.predictor,
                    config=self.sp_cfg,
...
def build_predictor_mini(model_path: str | None, device_str: str = "cpu") -> Tuple[PredictorMini, "torch.device"]:
    if not HAS_TORCH:
        device = torch.device("cpu")
    else:
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu") if device_str == "auto" else torch.device(device_str)
    print(f"[SelfPlayMini] device = {device}")
    model = MiniBanqiNet()
    if HAS_TORCH:
        model = model.to(device)
    predictor = PredictorMini(model, device, model_path)
    if model_path and os.path.exists(model_path):
        print(f"[SelfPlayMini] 使用模型权重: {model_path}")
    else:
        print(f"[SelfPlayMini] 使用全新初始化网络")
