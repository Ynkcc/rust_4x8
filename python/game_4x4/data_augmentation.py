"""
data_augmentation.py — 4x4 暗棋训练数据的空间对称增强（独立模块，以导入方式使用）

针对 4x4 方盘提供 D4 对称群全部 8 个空间自同构：
    identity（恒等）/ rot90（顺时针90°）/ rot180（180°）/ rot270（逆时针90°）
    hflip（水平翻转）/ vflip（垂直翻转）/ diag（主对角线镜像）/ anti_diag（反对角线镜像）

4x4 是正方形棋盘，8 个对称全部保持棋盘形状（4x8 长盘只有 4 个）。

变换时同步映射：
  - board 特征张量（(16, 4, 4) 或扁平 256 维）沿对应空间轴翻转；
  - 策略分布 / 动作掩码（112 维）按动作索引置换（与 Rust src/game_env/actions.rs 的
    build_action_lookup_tables 逐条一致）；
...
# ============================================================================
# 常量（与 python/game_4x4/constant.py、src/game_env/game4x4.rs 保持一致）
# ============================================================================
BOARD_ROWS = 4
BOARD_COLS = 4
...
# 全部对称变换（4x4 方盘 D4 群，共 8 个空间自同构）
SYMMETRY_TRANSFORMS: Tuple[str, ...] = (
    "identity", "rot90", "rot180", "rot270", "hflip", "vflip", "diag", "anti_diag",
)
# 默认增强所用变换（排除恒等，避免无效复制）
NON_IDENTITY_TRANSFORMS: Tuple[str, ...] = (
    "rot90", "rot180", "rot270", "hflip", "vflip", "diag", "anti_diag",
)
# 对合变换（perm[perm] == identity）；rot90 / rot270 需 4 次还原
INVOLUTIONS: Tuple[str, ...] = ("rot180", "hflip", "vflip", "diag", "anti_diag")
...
def _sq_map(transform: str) -> np.ndarray:
    """返回 16 元素映射表 map，满足 board_new.flat[i] = board_flat[map[i]]。

    即 map[i] 表示「变换后棋盘位置 i 对应的原格子索引」。
    推导：新位置 (r, c) 应取原位置 (pr, pc) 的内容。
    """
    if transform not in SYMMETRY_TRANSFORMS:
        raise ValueError("未知对称变换 %r，可选: %r" % (transform, SYMMETRY_TRANSFORMS))
    r = np.arange(BOARD_ROWS, dtype=np.int64)[:, None]
    c = np.arange(BOARD_COLS, dtype=np.int64)[None, :]
    ...
    if transform == "identity":
        ...
        pr, pc = c, rows - 1 - r
    elif transform == "hflip":       # 左右镜像
        pr, pc = r, cols - 1 - c
    elif transform == "vflip":       # 上下镜像
        pr, pc = rows - 1 - r, c
...
def action_permutation(transform: str) -> np.ndarray:
    """返回动作索引置换表 perm（长度 112），满足 new_policy = old_policy[perm]。

    等价地：对原动作 a 施加变换后得到的动作索引为 perm[a]（perm 即变换函数）。
    rot180 / hflip / vflip / diag / anti_diag 均为对合（perm[perm] == identity），
    rot90 与 rot270 互为逆，4 次应用还原。
    """
...
def transform_board(board: np.ndarray, transform: str) -> np.ndarray:
    """对棋盘张量应用空间变换。支持 (16,4,4) 或扁平 (256,) 输入，形状不变。"""
    if transform == "identity":
        return board
    arr = np.asarray(board)
    flat_in = arr.reshape(BOARD_CHANNELS, TOTAL_POSITIONS)
    m = _sq_map(transform)
    return flat_in[:, m].reshape(arr.shape)


def transform_policy(policy: np.ndarray, transform: str) -> np.ndarray:
    """对策略分布 / 动作掩码（112 维）按动作索引置换。"""
    if transform == "identity":
        return policy
...
# ============================================================================
# 自检（python python/game_4x4/data_augmentation.py）
# ============================================================================
if __name__ == "__main__":
    ...
    for t in SYMMETRY_TRANSFORMS:
        ...
        # 2) 对合性：对合变换应用两次还原；rot90/rot270 应用四次还原
        if t in INVOLUTIONS:
            assert (perm[perm] == np.arange(ACTION_SPACE_SIZE)).all(), t + " 非对合"
        else:
            four = perm[perm][perm][perm]
            assert (four == np.arange(ACTION_SPACE_SIZE)).all(), t + " 非 4 次还原"
        # 3) board 变换与坐标映射一致
        ...
        if t in INVOLUTIONS:
            ...
            four = transform_policy(transform_policy(transform_policy(
                transform_policy(policy, t), t), t), t)
            assert np.allclose(four, policy), t + " policy 非 4 次还原"
        # 5) mask 变换保持求和不变
        assert abs(mask[perm].sum() - mask.sum()) < 1e-5, t + " mask 和变化"

    # 6) 翻棋动作映射抽查（4x4 棋盘，hflip/rot180/rot90 示例）
    assert transform_action(5, "hflip") == 6      # (1,1) -> (1,2)
    assert transform_action(1, "rot180") == 14    # (0,1) -> (3,2)
    assert transform_action(1, "rot90") == 7      # (0,1) 顺旋 90° -> (1,3)

    # 7) 样本级增强（episode_to_samples 格式）
    sample = {
        "board_state": board.copy(),
        "scalar_state": np.random.rand(19).astype(np.float32),
    ...
    episode = {
        ...
        "scalars": [np.random.rand(19).astype(np.float32)] * 2,
