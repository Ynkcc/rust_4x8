"""
archiver.py — 后台异步冷存储归档线程（4x4 训练用）

从归档队列批量消费 episode，异步写入 MongoDB（作为冷存储），
缺 pymongo 或连接失败时降级为本地 JSONL 归档。不阻塞自对弈与训练主流程。

本地归档目录独立于 4x8 正式训练（training_data/archive_4x4），避免数据混写。
"""
...
LOCAL_ARCHIVE_DIR = os.getenv("G4X4_ARCHIVE_DIR", "./training_data/archive_4x4")
...
class ArchiverWorker(threading.Thread):
    ...
    def __init__(
        ...
        super().__init__(name="ArchiverWorker4x4", daemon=True)
    ...
    def _init_saver(self):
        ...
        if self.mongo_uri:
            try:
                ...
                print(f"[Archiver4x4] ⚠️ MongoDB 连接失败，降级为本地归档: {exc}")
    ...
    def _flush(self) -> None:
        ...
        try:
            ...
            print(f"[Archiver4x4] ⚠️ 归档失败（将丢弃本批）: {exc}")
        finally:
            self.pending = []

    def run(self) -> None:
        print(f"[Archiver4x4] 🗄️  归档线程启动（Mongo={bool(self.mongo_uri)}, "
        ...
        if hasattr(self.saver, "close"):
            self.saver.close()
        print(f"[Archiver4x4] 归档线程退出，累计归档 {self.archived_games} 局")
