...
2.  **MCTS Sensitivity**:
    *   **Do NOT remove** the header warning in `src/mcts.rs`.
...
4.  **Bitboards**: Use bitwise operations for board logic in `src/game_env.rs`. Avoid object allocations in hot paths.
...
*   **Components**:
    *   `src/mcts.rs`: **Gumbel AlphaZero MCTS** implementation (Top-K sampling, Sequential Halving).
    *   `src/self_play.rs`: Manages game loops and data collection.
    *   `src/game_env.rs`: The core game logic (4x8 board, bitboards).
...
| :--- | :--- | :--- |
| **Env** | `src/game_env.rs` | 4x8 Board, Bitboards, Rules. `ACTION_SPACE_SIZE=352`. |
| **MCTS** | `src/mcts.rs` | Gumbel MCTS. **Touch with care.** |
| **Collector** | `src/data_collector.rs`| Main binary for self-play. Connects to MongoDB. |
| **Evaluator** | `src/data_collector.rs`| `LocalEvaluator` implements `Evaluator` trait using `tch-rs`. |
