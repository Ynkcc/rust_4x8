...
use banqi_4x8::mcts::{Evaluator, MCTSConfig, MCTS};
...
fn save_samples_to_db(
    ...
    let tx = conn.transaction()?;
    {
        ...
        for (obs, probs, value, mask) in samples {
            ...
            let probs_bytes: Vec<u8> = probs.iter().flat_map(|&x| x.to_le_bytes()).collect();

            let mask_bytes: Vec<u8> = mask.iter().flat_map(|&x| x.to_le_bytes()).collect();
...
fn print_db_stats(conn: &Connection) -> Result<()> {
    let total: i64 = conn.query_row("SELECT COUNT(*) FROM training_samples", [], |row| {
        row.get(0)
    })?;
    ...
    println!(
        "自对弈样本: {} ({:.1}%)",
        self_play,
        self_play as f64 / total as f64 * 100.0
    );
    println!(
        "场景样本: {} ({:.1}%)",
        scenario,
        scenario as f64 / total as f64 * 100.0
    );
...
fn load_samples_from_db(conn: &Connection) -> Result<Vec<(Observation, Vec<f32>, f32, Vec<i32>)>> {
    ...
    let samples = stmt.query_map([], |row| {
        ...
        // 将字节转换回f32数组
        let board_data: Vec<f32> = board_bytes
            .chunks_exact(4)
            .map(|chunk| f32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]))
            .collect();

        let scalar_data: Vec<f32> = scalar_bytes
            .chunks_exact(4)
            .map(|chunk| f32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]))
            .collect();

        let probs: Vec<f32> = probs_bytes
            .chunks_exact(4)
            .map(|chunk| f32::from_le_bytes([chunk[0], chunk[1], chunk[2], chunk[3]]))
            .collect();

        let mask: Vec<i32> = mask_bytes
            .chunks_exact(4)
        ...
        use ndarray::Array;
        let board =
            Array::from_shape_vec((2, 8, 3, 4), board_data).expect("Failed to reshape board data");
...
fn collect_scenario_samples(
    ...
    // 场景1: R_A vs B_A
    {
        ...
        loop {
            ...
            // 打印MCTS策略
            if step == 0 {
                let mut prob_actions: Vec<(usize, f32)> = probs
                    .iter()
                    .enumerate()
                    .filter(|(_, &p)| p > 0.01)
                    .map(|(i, &p)| (i, p))
    ...
    // 场景1: 仅剩 R_A 与 B_A
    {
        ...
        if iteration <= 2 {
            let logits_vec: Vec<f32> = (0..46)
                ...
                .collect();
            let top_logits: Vec<(usize, f32)> = logits_vec
                .iter()
                .enumerate()
                .map(|(i, &l)| (i, l))
                .collect::<Vec<_>>();
            ...
            println!(
                "    [Debug] action38 logit={:.2}, action39 logit={:.2}",
        ...
        let probs_vec: Vec<f32> = (0..46)
            .map(|i| probs.double_value(&[0, i]) as f32)
            .collect();
    ...
    // 场景2: 隐藏的威胁
    {
        ...
        if iteration <= 2 {
            let logits_vec: Vec<f32> = (0..46)
                ...
                .collect();
            println!(
                "    [Debug] action3 logit={:.2}, action5 logit={:.2}",
...
pub fn train_loop() -> Result<()> {
    ...
    println!(
        "训练配置: iterations={}, episodes={}, mcts_sims={}, batch_size={}, epochs={}",
    ...
    if !existing_samples.is_empty() {
        ...
        for epoch in 0..initial_training_epochs {
            let (loss, p_loss, v_loss) = train_step(
                &mut opt,
                &evaluator.net,
                &existing_samples,
                batch_size,
        ...
        println!(
            "  初始训练平均Loss: {:.4} (Policy={:.4}, Value={:.4})",
            avg_loss, avg_p_loss, avg_v_loss
        );
    ...
    for iteration in 0..num_iterations {
        ...
        for eps in 0..num_episodes {
            ...
            loop {
                ...
                if iteration == 0 && eps == 0 && episode_step < 3 {
                    ...
                    prob_actions.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap());
                    println!(
                        "    [Debug] Episode 0, Step {}, MCTS策略 Top-3: {}",
                        episode_step,
                        prob_actions
                            .iter()
                            .take(3)
                    ...
                    Ok((_, _, terminated, truncated, winner)) => {
                        ...
                        if terminated || truncated {
                            ...
                            if iteration == 0 && eps == 0 {
                                println!(
                                    "    [Debug] Episode 0 结束: 步数={}, 胜者={:?}, reward_red={}",
        ...
        if iteration <= 2 {
            ...
            for i in 0..3.min(examples.len()) {
                ...
                let max_action = probs
                    ...
                    .unwrap_or(0);
                println!(
                    "  [Debug] 样本{}: value={:.3}, max_action=a{}, max_prob={:.3}, entropy={:.3}",
            ...
            println!(
                "  [Debug] 训练集中: action38主导的样本={}, action3主导的样本={}",
        ...
        for epoch in 0..epochs_per_iteration {
            ...
            if (epoch + 1) % 2 == 0 || epoch == epochs_per_iteration - 1 {
                println!(
                    "  Training: Epoch {}/{}, Loss={:.4} (Policy={:.4}, Value={:.4})",
                    epoch + 1,
