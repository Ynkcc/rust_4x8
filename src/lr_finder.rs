...
pub fn find_learning_rate(
    ...
    for step in 0..config.num_steps {
        ...
        if step % 10 == 0 || step == config.num_steps - 1 {
            println!(
                "  Step {}/{}: LR={:.2e}, Loss={:.4} (Policy={:.4}, Value={:.4})",
                step + 1,
                config.num_steps,
                lr,
                smooth_loss,
...
fn save_results_to_csv(results: &[LRFinderResult], path: &str) -> Result<()> {
    ...
    for result in results {
        writeln!(
            file,
            "{:.8e},{:.6},{:.6},{:.6}",
            result.learning_rate, result.loss, result.policy_loss, result.value_loss
        )?;
    }
...
fn analyze_and_suggest(results: &[LRFinderResult]) -> Result<()> {
    ...
    println!(
        "  - 循环学习率: 在 {:.2e} 和 {:.2e} 之间循环",
        suggested_min_lr, suggested_max_lr
    );
    println!(
        "  - Adam 优化器: 当前使用 Adam，建议起始学习率 {:.2e}",
        suggested_initial_lr
    );
