...
use crate::local_evaluator::LocalEvaluator;
use crate::py::{PyGameEpisode, PySelfPlayConfig};
use crate::self_play::{self, SelfPlayConfig};
use tch::Device;
...
/// 运行并行自对弈（rayon 多线程，纯 Rust），返回非空 episode。
fn run_parallel_by_variant(
    py: Python<'_>,
    eval: &RustEvaluator,
    cfg: &SelfPlayConfig,
    num_workers: usize,
    ...
    let predict_fn_per_worker: Vec<_> = (0..num_workers.max(1)).collect();
    let pool = rayon::ThreadPoolBuilder::new()
        .num_threads(num_workers.max(1))
        .build()
        .expect("failed to build rayon thread pool for rust parallel self_play");

    let episodes_by_worker: Vec<Vec<PyGameEpisode>> = py.detach(|| {
        pool.install(|| {
            use rayon::prelude::*;
            predict_fn_per_worker
                ...
                .map(|_| {
                    ...
                    // 每个 worker 共享同一个 evaluator（Arc 引用计数共享，不复制模型）。
                    match eval {
                        RustEvaluator::Dark(e) => {
                            for _ in 0..games_per_worker {
                                let ep = self_play::run_self_play(e, cfg, DarkChessEnv::new);
                                if !ep.samples.is_empty() {
                                    local.push(PyGameEpisode { inner: ep, variant });
                                }
                            }
                        }
                        RustEvaluator::Mini(e) => {
                            for _ in 0..games_per_worker {
                                let ep = self_play::run_self_play(e, cfg, MiniDarkChessEnv::new);
                                if !ep.samples.is_empty() {
                                    local.push(PyGameEpisode { inner: ep, variant });
                                }
                            }
                        ...
                        RustEvaluator::Game4x4(e) => {
                            for _ in 0..games_per_worker {
                                let ep = self_play::run_self_play(e, cfg, Game4x4Env::new);
                                if !ep.samples.is_empty() {
                                    local.push(PyGameEpisode { inner: ep, variant });
                                }
                            }
    ...
    episodes_by_worker.into_iter().flatten().collect()
}

/// 运行串行自对弈（单线程，纯 Rust），返回非空 episode。
fn run_serial_by_variant(
    py: Python<'_>,
    eval: &RustEvaluator,
    cfg: &SelfPlayConfig,
    num_games: usize,
    ...
    let variant = eval.variant();
    let _ = worker_id;
    let games: Vec<self_play::GameEpisode> = py.detach(|| match eval {
        RustEvaluator::Dark(e) => {
            (0..num_games)
                .map(|_| self_play::run_self_play(e, cfg, DarkChessEnv::new))
                .collect()
        }
        RustEvaluator::Mini(e) => {
            (0..num_games)
                .map(|_| self_play::run_self_play(e, cfg, MiniDarkChessEnv::new))
                .collect()
        }
        RustEvaluator::Game4x4(e) => {
            (0..num_games)
                .map(|_| self_play::run_self_play(e, cfg, Game4x4Env::new))
                .collect()
        }
    });
    games
...
impl RustTorchCollector {
    ...
    /// 并行自对弈（rayon 多线程）：每个 worker 共享同一份 Rust 侧模型，
    /// 推理真正并行（无 GIL 串行化）。
    #[pyo3(signature = (config, num_workers=4, games_per_worker=1, worker_id=0))]
    fn run_parallel(
        &self,
        py: Python<'_>,
        config: Option<PyRef<PySelfPlayConfig>>,
        ...
        let eval = self.eval.read().unwrap().clone();
        Ok(run_parallel_by_variant(
            py, eval.as_ref(), &cfg, num_workers, games_per_worker, worker_id,
        ))
    }

    /// 串行自对弈（单线程，纯 Rust）。
    #[pyo3(signature = (config, num_games=1, worker_id=0))]
    fn run_serial(
        &self,
        py: Python<'_>,
        config: Option<PyRef<PySelfPlayConfig>>,
        ...
        let eval = self.eval.read().unwrap().clone();
        Ok(run_serial_by_variant(
            py, eval.as_ref(), &cfg, num_games, worker_id,
        ))
    }
}
