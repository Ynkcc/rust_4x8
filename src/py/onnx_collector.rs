...
use crate::onnx::{OnnxEvaluator, OnnxModel};
...
fn run_parallel_by_variant(
    ...
    let worker_indices: Vec<usize> = (0..num_workers.max(1)).collect();
    let pool = rayon::ThreadPoolBuilder::new()
        .num_threads(num_workers.max(1))
        .build()
        .expect("failed to build rayon thread pool for rust onnx parallel self_play");
