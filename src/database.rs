...
pub fn load_samples_from_db(
    conn: &Connection,
) -> Result<Vec<(Observation, Vec<f32>, f32, Vec<i32>)>> {
...
/// 加载距离游戏结束指定步数以内的样本
pub fn load_endgame_samples_from_db(
    conn: &Connection,
    max_steps_from_end: usize,
) -> Result<Vec<(Observation, Vec<f32>, f32, Vec<i32>)>> {
    let mut stmt = conn.prepare(
    ...
    let samples = stmt.query_map([max_steps_from_end as i64], |row| {
        ...
        let board =
            Array::from_shape_vec((1, 8, 3, 4), board_data).expect("Failed to reshape board data");
        let scalars = Array::from_vec(scalar_data);
