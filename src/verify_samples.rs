...
fn main() -> Result<()> {
    ...
    let samples = stmt.query_map([], |row| {
        ...
        // 调试：打印实际数据长度
        eprintln!(
            "🐛 DEBUG: board_data.len()={}, scalar_data.len()={}, probs.len()={}, mask.len()={}",
            board_data.len(),
            scalar_data.len(),
            probs.len(),
            mask.len()
...
fn print_observation(obs: &Observation) {
    ...
    if obs.scalars.len() >= 56 {
        ...
        println!(
            "  前10个特征: {:?}",
            &scalars_data[..10.min(scalars_data.len())]
        );
    }
}
                ...
                for (dir_idx, (dr, dc)) in moves.iter().enumerate() {
                    ...
                    if r2 >= 0 && r2 < 3 && c2 >= 0 && c2 < 4 {
                        if idx == action {
                            ...
                            return format!(
                                "(move [{},{}]->{} to [{},{}])",
                                (b'A' + r1 as u8) as char,
                                c1,
                                dir_names[dir_idx],
                                (b'A' + r2 as u8) as char,
                                c2
                            );
                        }
                        idx += 1;
